...
class Qwen35PatchEmbed(nn.Module):
    ...
    loader to avoid needing a Conv3D op (no TVM/Metal support). See qwen3_5v_loader.py.
    """
...
class Qwen35PatchMerger(nn.Module):
    ...
    With use_postshuffle_norm=False (default), norm is LayerNorm(hidden_size)
    applied per-patch before the spatial merge reshape.
    """
