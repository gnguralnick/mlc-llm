...
from .qwen3_5_vision import Qwen35VisionConfig, Qwen35VisionModel
...
class Qwen35VForCausalLM(nn.Module):
    ...
    def prefill(self, input_embed: Tensor, state: HybridState):
        return self.language_model.prefill(input_embed, state)

    def decode(self, input_embed: Tensor, state: HybridState):
        return self.language_model.decode(input_embed, state)

    def batch_prefill(
        self, input_embeds: Tensor, logit_positions: Tensor, state: HybridState
    ):
        return self.language_model.batch_prefill(input_embeds, logit_positions, state)

    def batch_decode(self, input_embeds: Tensor, state: HybridState):
        return self.language_model.batch_decode(input_embeds, state)

    def batch_verify(self, input_embeds: Tensor, state: HybridState):
        return self.language_model.batch_verify(input_embeds, state)
    ...
    def create_paged_kv_cache(
        ...
        return self.language_model.create_paged_kv_cache(
            max_batch_size, max_total_seq_len, prefill_chunk_size,
